...
const GradientOrb: React.FC<{ hex: string; index: number }> = ({ hex, index }) => {
  const frame = useCurrentFrame();

  const orbX = interpolate(
    frame,
    [0, 75],
    [200 + index * 350, 250 + index * 350 + Math.sin(index * 2) * 50]
  );
  const orbY = interpolate(
    frame,
    [0, 75],
    [800 + index * 30, 750 + Math.cos(index) * 100]
  );
...
export const ColorsScene: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Color swatches with motion blur */}
      <div
        style={{
          display: "flex",
          gap: 30,
      ...
      >
        {palette.map(({ name, hex }, i) => (
          <Trail key={name} lagInFrames={3} trailOpacity={0.5} layers={4}>
            <ColorSwatch name={name} hex={hex} index={i} />
          </Trail>
        ))}
      </div>

      {/* Animated gradient orbs with motion blur */}
      {palette.map(({ hex }, i) => (
        <Trail key={i} lagInFrames={4} trailOpacity={0.4} layers={3}>
          <GradientOrb hex={hex} index={i} />
        </Trail>
      ))}
    </AbsoluteFill>
  );
